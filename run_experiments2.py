...
def generate_parameter_grid(num_params=10**2):
    ...
    # loguniform distribution of aperture sizes between 3 and 100 scale radii
    ...
    return np.c_[gammas, backgrounds, apertures, Ncluster, res, count_photons]


def main():
    if not isdir("results2"):
        os.mkdir("./results2")

    if glob("results2/*.npy"):
        i = (
            max(
                [
                    int(f.split("results2/")[1].split(".npy")[0])
                    for f in glob("results2/*.npy")
                ]
            )
            + 1
        )
    ...
    while True:
        params = generate_parameter_grid(10**3)
        result = np.array(
            Parallel(35)(delayed(inference_experiment)(*p) for p in params)
        )
        #        print(np.array(params).shape, result.shape)
        np.save(f"results2/{i}.npy", np.c_[params, result])
        #        pickle.dump(np.c_[params, result], open(f"results2/{i}.dump", "wb"))
        i += 1
